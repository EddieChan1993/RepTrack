...
- **每日邮件提醒** — 通过 SMTP 直接发送今日推荐、今日已复习、昨日复习内容，无需系统邮件客户端；支持 QQ / 163 / Gmail / Outlook 快捷预设；支持开启「每天自动发送」并自定义发送时间点，配置保存在本机，不随数据文件同步，多台电脑各自独立
- **自动备份** — 可设置每天定时自动备份，最多保留10个备份文件；支持从备份列表一键恢复或从文件选择恢复，恢复前弹确认框防止误操作
- **课时置灰** — hover 课时芯片右上角出现红色禁止图标，点击将该课时置灰并从所有统计和推荐中剔除；置灰后显示蓝色恢复图标可随时恢复
